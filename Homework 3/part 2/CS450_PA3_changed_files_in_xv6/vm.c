...
int is_PA_linked_with_sharedMemory(uint);
...
// Added
void forced_free_phys_page(void *);
...
struct shared_memory_region {
  ...
  int len;
  int ref_count;
  uint phys_pages[MAX_REGION_SIZE];
};

struct shared_memory_region regions[64];

/**
* Maps region "key" into process "p"
* at virtual addres "addr"
*/
void map_shared_memory_region(int key, struct proc *p, void *virt_addr) {
  for(int k = 0; k < regions[key].len; k++) {
    mappages(p->pgdir, (void*)(virt_addr + (k * PGSIZE)), PGSIZE, regions[key].phys_pages[k], PTE_W | PTE_U);
  }
}
...
/** Copy the shared memory regions
* of process "p" into process "new_p"
*/
int copy_shared_memory_regions(struct proc *p, struct proc *new_p) {
  for(int i = 0; i < (int)(sizeof(p->shared_memory) / sizeof(p->shared_memory[0])); i++) {
    if(p->shared_memory[i].key != -1) {
      new_p->shared_memory[i] = p->shared_memory[i];
      int key = new_p->shared_memory[i].key;
      regions[key].ref_count++;
      map_shared_memory_region(key, new_p, new_p->shared_memory[i].virt_addr);
    }
  }
  return 0;
}

/** Syscall that removes the calling process
* from accessing the shared pages
...
FreeSharedPage(int key)
{
  ...
  void *virt_addr = 0;
  for(int i = 0; i < (int)(sizeof(p->shared_memory) / sizeof(p->shared_memory[0])); i++){
    if(p->shared_memory[i].key == key) {
      virt_addr = p->shared_memory[i].virt_addr;
      p->shared_memory[i].key = -1;
      p->shared_memory[i].virt_addr = 0;
      break;
    }
  }
  if(virt_addr == 0){
  ...
  // Clear page table entries (pte)
  struct shared_memory_region* reg = &regions[key];

  for(int i = 0; i < reg->len; i++) {
    pte_t* pte = walkpgdir(p->pgdir, (char*)virt_addr + i * PGSIZE, 0);
    if(pte == 0) {
      return -1;
    }
    *pte = 0;
  ...
  // Decrease the reference count; free if ref_count isn't used
  reg->ref_count--;
  if(reg->ref_count == 0) {
    regions[key].valid = 0;
    regions[key].ref_count = 0;
    for(int i = 0; i < regions[key].len; i++){
      kfree(P2V(regions[key].phys_pages[i]));
    }
    regions[key].len = 0;
  }
  return 0;
...
/**
* Check if given PA (phys_addr)
* is linked with a shared memory page
*/
int is_PA_linked_with_sharedMemory(uint phys_addr){
  struct proc *p = myproc();
  for(int i = 0; i < (int)(sizeof(p->shared_memory) / sizeof(p->shared_memory[0])); i++) {
    for(int j = 0; j < regions[i].len; j++) {
      if(phys_addr == (uint)regions[i].phys_pages[j]) {
        return 1;
      }
    }
  }
...
/**
* Free pages when
* is_PA_linked_with_sharedMemory (are not shared)
* fails (returns 0)
*/
void forced_free_phys_page(void *phys_addr) {
  if(is_PA_linked_with_sharedMemory(V2P(phys_addr)) == 0){
    kfree(phys_addr);
   }
}
